  ...
  return (
    <div className='h-screen bg-[#FBD579] w-full'>
      ...
      <div className='h-3/6 w-full flex items-start justify-center relative'>
        {page === 1 && <LandingPage1 style={{ height: "120%" }} />}
        {page === 2 && <LandingPage2 style={{ height: "120%" }} />}
        {page === 3 && <LandingPage3 style={{ height: "120%" }} />}

        {(page === 2 || page === 3) && <div className='absolute top-48 left-6'>
          <Left style={{ height: "100%" }} onClick={prevPage} />
        </div>}
        {(page === 1 || page === 2) && <div className='absolute top-48 right-6'>
          <Right style={{ height: "100%" }} onClick={nextPage} />
        </div>}
      </div>
      <div className='h-2/6 w-full flex items-center justify-center'>
        <div className='w-10/12'>
          {page === 3 && <FormButton text='はじめる' />}
        </div>
      </div>
    </div>
  );
